...
/*   Updated: 2025/06/29 11:24:13 by aakroud          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_cmd_word_check(t_tree *tree)
{
	...
	// 	return (0);
	if (!ft_strcmp("COMMAND", tree->type) || !ft_strcmp("WORD", tree->type) || !ft_strcmp("REDIRECTION", tree->type))
		return (0);
	return (1);
}
void	ft_cmd_word_helper(t_tree *tree, t_hdoc_data *h_data, int *check, char **e)
{
	if ((ft_strcmp("COMMAND", tree->type) == 0 && tree->redirections == NULL)
		|| (ft_strcmp("WORD", tree->type) == 0 && cmd_check(tree) == 0))
	{
...
{
	if (!ft_strcmp("WORD", tree->type) && !cmd_check(tree) && !tree->redirections)
	{
		dprintf(2, "yes baby\n");
		reset_vars(&tree, h_data->env, h_data);
		tree->status = ft_cmd_exec(tree, h_data);
	...
	else if (!ft_strcmp("WORD", tree->type) && !cmd_check(tree) && tree->redirections)
	{
		ft_execute_com(tree, h_data);
		ft_check_exit(check, tree->status);
	}
