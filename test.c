...
char *side_maker(t_list **head, int number, int j) {
    ...
    while (i < number - j) {
        ...
        // Add space between tokens (use " " instead of "")
        if (tmp->next && 
            !(ft_strcmp(tmp->token, "REDIRECTION") == 0 && 
              ft_strcmp(tmp->next->token, "WORD") == 0)) {
            tmp_char = tmp2;
            tmp2 = ft_strjoin_leak(tmp2, " ", 1444); // Change this from "" to " "
...
char *extract_command_with_redirects(t_list **head, t_list **pipe_pos)
{
    ...
    *pipe_pos = NULL;
    ...
    while (current) {
        ...
        // Build the full command string up to the pipe/operation WITH SPACES
        if (command_str) {
            temp_str = command_str;
            command_str = ft_strjoin_leak(command_str, " ", __LINE__); // Add a SPACE here instead of empty string
